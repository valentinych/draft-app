...
def get_roster_for_gw(state: Dict[str, Any], manager: str, target_gw: int) -> List[Dict[str, Any]]:
    ...
    # 
    # Логика:
    # - Трансфер в GW N применяется с GW N+1
    # - Текущий ростер содержит все трансферы, включая те, что были сделаны в GW < target_gw
    # - Для target_gw нужно показать ростер, который был актуален для этого GW
    # - Трансфер в GW10 применяется с GW11, поэтому:
    #   * Для GW10: трансфер из GW10 еще не применился, нужно откатить (transfer_gw >= target_gw)
    #   * Для GW11: трансфер из GW10 уже применился, НЕ нужно откатывать (transfer_gw < target_gw)
    # - Значит: откатываем трансферы где transfer_gw >= target_gw
    rollback_transfers = []
