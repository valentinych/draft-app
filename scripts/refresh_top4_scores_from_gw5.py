#!/usr/bin/env python3
"""
Refresh Top-4 draft scores starting from GW5
Updates player statistics from API Football (if enabled) or MantraFootball
and recalculates scores for all gameweeks from GW5 onwards

Usage:
    python3 scripts/refresh_top4_scores_from_gw5.py
    or
    heroku run --app val-draft-app "python3 scripts/refresh_top4_scores_from_gw5.py"
"""
import sys
import os
...
def get_all_rounds_from_gw5():
    """Get all rounds starting from GW5"""
...
def main():
    print("=" * 80)
    print("ОБНОВЛЕНИЕ ОЧКОВ TOP-4 ДРАФТА С GW5")
    print("=" * 80)
    ...
    # Get all rounds from GW5
    rounds_to_refresh = get_all_rounds_from_gw5()
    ...
    print(f"✅ Найдено уникальных игроков: {len(all_player_ids)}")
    ...
    # Clear lineup caches for GW5 and above
    print(f"\n🗑️  Очистка кеша лайнапов для GW5 и выше...")
    cleared_count = 0
    ...
    print("   чтобы увидеть обновленные очки начиная с GW5")
